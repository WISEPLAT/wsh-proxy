...
def on_shutdown(f):
    ...
    if os.path.isfile('wsh-proxy.pid'):
        os.remove('wsh-proxy.pid')
...
def on_connect(f):
    ...
    initial_job = (yield f.rpc('wsh_submitLogin', [settings.WALLET, settings.CUSTOM_EMAIL], 'Proxy_'+version.VERSION+debug))
...
def main():
    ...
    log.warning("Wiseplat Stratum proxy version: %s" % version.VERSION)
